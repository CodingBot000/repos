...
요건에는 없지만 강의를 시간표에 등록한 상태에서 앱을 삭제후 재 설치시 로컬에 데이터가 없어서 시간표는 비어있지만 삭제전 등록한 강의를 추가하려고하면 이미 등록된 강의라고 나오면서 영원히 추가를 못하게됩니다.
...
동기화 처리 프로세스가 다소 비효율적인 부분이 있는데 내가 등록한 시간표정보를 요청시 강의코드만 돌려주기 때문에 어느정도는 불가피하였습니다.
